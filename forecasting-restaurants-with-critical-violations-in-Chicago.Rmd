...
At this time, the food inspection database is not hosted by the City of Chicago, instead, a file of all food inspections are sent to the City of Chicago open data team on a daily basis, which is automatically uploaded to the portal every morning. 

In addition, the City also publishes other relevant data:
...
Thus, the principal question for researchers is whether this analytical model provides more efficiency for the food inspection team. CDPH operational procedures requires the department to inspect every risk 1 and risk 2 restaurant. Therefore, the operational goal is to allow inspectors to discover critical violations earlier than their current operations.

## Evaluation Design

The analytical model was trained on data from January 2011 through January 2014. The coefficients were established by the relationships during this period. 

Researchers observed the inspections completed by CDPH from ```r dat[iiTest, range(Inspection_Date)][1]``` to ```r dat[iiTest, range(Inspection_Date)][2]``` and calculate the percentage of inspections that result in critical violations in the first half of the inspections during this period. CDPH conducted the inspections at their own accord without any intervention from the research team.
