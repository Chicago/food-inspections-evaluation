...
```{r}
m0_coef <- m0$beta[,ncol(m0$beta)]
m0_inspCoef <- m0_coef[grepl("^Inspector.Assigned",names(m0_coef))]
m1_coef <- m1$beta[,ncol(m1$beta)]
m1_inspCoef <- m1_coef[grepl("^Inspector.Assigned",names(m1_coef))]
m2_coef <- m2$beta[,ncol(m2$beta)]
m2_inspCoef <- m2_coef[grepl("^Inspector.Assigned",names(m2_coef))]
m3_coef <- m3$beta[,ncol(m3$beta)]
m3_inspCoef <- m3_coef[grepl("^Inspector.Assigned",names(m3_coef))]
...
rm(iEval, iTest, iTrain, ii, iiLambda, iiLambdaEval, iiLambdaTest, iiLambdaTrain, lambdaPredMatrix)
```

```{r}

plotLambdaAuc <- function(predMatrix = lambdaPredMatrix_m0[iiEval, ],
                          yy = yyEval, nbreaks = 100){
    for(i in 1:ncol(predMatrix)){
        if(i==1){
            iiTrunc <- trunc(seq(1, length(yy), length.out=nbreaks))
            plot(data.frame(c(0,1),c(0,1)), type="n", xlab="", ylab="")
            }
        iiLambda <- order(-predMatrix[,i])
        lines(x = c(1:length(iiTrunc))/length(iiTrunc),
              y = (cumsum(yy[iiLambda]) / sum(yy[iiLambda]))[iiTrunc] )
        }
...
```{r}
par(mfrow=c(1,3))
plotLambdaAuc(lambdaPredMatrix_m0[iiTrain, ], yyTrain)
plotLambdaAuc(lambdaPredMatrix_m0[iiTest, ], yyTest)
plotLambdaAuc(lambdaPredMatrix_m0[iiEval, ], yyEval)
par(mfrow=c(1,1))
```
...
plotLambdaAuc(lambdaPredMatrix_m1[iiTrain, ], yyTrain)
plotLambdaAuc(lambdaPredMatrix_m1[iiTest, ], yyTest)
plotLambdaAuc(lambdaPredMatrix_m1[iiEval, ], yyEval)

plotLambdaAuc(lambdaPredMatrix_m2[iiTrain, ], yyTrain)
plotLambdaAuc(lambdaPredMatrix_m2[iiTest, ], yyTest)
plotLambdaAuc(lambdaPredMatrix_m2[iiEval, ], yyEval)

plotLambdaAuc(lambdaPredMatrix_m3[iiTrain, ], yyTrain)
plotLambdaAuc(lambdaPredMatrix_m3[iiTest, ], yyTest)
plotLambdaAuc(lambdaPredMatrix_m3[iiEval, ], yyEval)
par(mfrow=c(1,1))
