...
##==============================================================================
dat <- readRDS("DATA/31_random_forest_data.Rds")
model <- readRDS("DATA/31_random_forest_model.Rds")
...
dat[Test==FALSE, gini(rf_pred_test, criticalFound, plot=TRUE)]
dat[Test==TRUE, gini(rf_pred_test, criticalFound, plot=TRUE)]

## Calculate confusion matrix values for evaluation
calculate_confusion_values(actual = dat[Test==TRUE, criticalFound],
                           expected = dat[Test==TRUE, rf_pred_test], 
                           r = .25)
...
## Mean time savings:
datTest[ , simulated_date_diff_mean(Inspection_Date, rf_pred_test, criticalFound)]

## Detailed time savings:
bins <- datTest[ , simulated_bin_summary(Inspection_Date, rf_pred_test, criticalFound)]
bins
...
# predTest <- prediction(datTest[ ,list(rf_pred_test, runif(.N) )], 
#                        datTest[ ,list(criticalFound, criticalFound)])

predTest <- prediction(datTest$rf_pred_test, datTest$criticalFound)
