...
```{r}
geneorama::set_project_dir("food-inspections-evaluation")
datmerge <- merge(dat[,.SD,list(inspection_id=as.character(inspection_id))], 
                  dat_prev3, by = "inspection_id", all=TRUE)
...
datmerge[pastCritical.x==2 & pastCritical.y==0, .N, keyby=list(doing_business_as_name.x, license)]
...
vio_mat
manualsummary <- data.table(inspection_id = temp$Inspection_ID,
